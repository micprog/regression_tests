...
{        
  ...
  jump_to_address(apb_soc_bootaddr_get());
...
{  
  ...
  // Jtag bits tell us what to do
  unsigned int value = apb_soc_jtag_reg_ext(apb_soc_jtag_reg_read());

  switch(apb_soc_bootsel_get(ARCHI_APB_SOC_CTRL_ADDR)){

    case 0:
       bootFromRom(0, 1); //SPI flash
       break;
    ...
    case 1:
       if(value){
         ...
         boot_abort(); // The debug module takes over the boot process
       }
       break;

    case 2:
       bootFromRom(1, 0); //HYPER flash
       break;
